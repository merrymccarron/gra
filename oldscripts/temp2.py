...
#these are Ravi's credentials, not the CUSP ones we've been using on other API calls.
...
def get_follower_details():
    followersDF = pd.read_csv('sociallandmarks/NYPDNewsfollowerids.csv')
...
#checks to see if there's already a file with followers for this user -- if not, creates new csv file
    if not os.path.isfile('sociallandmarks/NYPDNewsfollowerdetails.csv'):
        df.to_csv('sociallandmarks/NYPDNewsfollowerdetails.csv', header = True, columns = ['UserID', 'location', 'screen_name', 'name', 'geo_enabled'], engine='python')
    ...
    while endOfCurrSlice < numberOfFollowers:
        try:
            ...
            with open('sociallandmarks/NYPDNewsfollowerdetails.csv', 'a') as f:
