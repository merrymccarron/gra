...
def get_follower_details():
    followersDF = pd.read_csv('sociallandmarks/FDNYfollowerids.csv')
...
#checks to see if there's already a file with followers for this user -- if not, creates new csv file
    if not os.path.isfile('sociallandmarks/FDNYfollowerdetails.csv'):
        df.to_csv('sociallandmarks/FDNYfollowerdetails.csv', header = True, columns = ['UserID', 'location', 'screen_name', 'name', 'geo_enabled'], engine='python')
    ...
    while endOfCurrSlice < numberOfFollowers:
        try:
            ...
            with open('sociallandmarks/FDNYfollowerdetails.csv', 'a') as f:
