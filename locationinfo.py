...
for csvfile in glob.glob('sociallandmarks/*details.csv'):
    try:
        ...
        #an error I was getting from pandas due to new lines in the location field
        ...
        #     value='None')
        tempdf = pd.read_csv(csvfileUniv, usecols=['UserID', 'location'], warn_bad_lines = True, engine='python').fillna(
            value='None')
        print "appending"
        df = df.append(tempdf)
...
df.to_csv('sociallandmarks/slfollowersNYbyLocationTextMAYBE.csv')

newYorkerUserIDmatrix.to_csv('newyorkeryesnomatrixMAYBE.csv', index_label='index')
