...
def followersslice(listofFollowers, apicallcount):
...
def getFollowerDetails():
	followersDF = pd.read_csv('sociallandmarks/NYCMayorsOfficefollowerids.csv')
...
#checks to see if there's already a file with followers for this user -- if not, creates new csv file
	if not os.path.isfile('sociallandmarks/NYCMayorsOfficefollowerdetails.csv'):
		df.to_csv('sociallandmarks/NYCMayorsOfficefollowerdetails.csv')

	while endOfCurrSlice < numberOfFollowers:
		try:
			useridlistSlice = followersslice(listofFollowers, apicallcount)
			requestUrl = baseApiUrl + useridlistSlice
			queryResults = oauth_req(requestUrl)
			data = queryResults
			tempdf = pd.DataFrame(data=data)
			with open('sociallandmarks/NYCMayorsOfficefollowerdetails.csv', 'a') as f:
